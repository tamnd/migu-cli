...
describes what users get. Pick a format with `-o`, or let migu choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
migu <command> -o table   # aligned columns for reading
migu <command> -o jsonl   # one JSON object per line, for piping
migu <command> -o json    # a single JSON array
migu <command> -o csv     # spreadsheet friendly
migu <command> -o tsv     # tab-separated
migu <command> -o url     # just the URL column
migu <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
migu <command> --fields id,title,url
```
...
```bash
migu <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
migu <command>            # a table, because this is a terminal
migu <command> | wc -l    # JSONL, because this is a pipe
```
